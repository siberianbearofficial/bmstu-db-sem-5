...
class Enrollment:
    def __init__(self, student_id: UUID, course_id: UUID, expires_at: datetime, comment: str | None = None):
        self.id = uuid4()
    ...
    def list(self):
        return [self.id, self.comment, self.student_id, self.course_id, self.created_at, self.expires_at]
...
class EnrollmentFaker:
    ...
    def create_enrollments(self, student_ids: list[UUID], course_ids: list[UUID], count: int = 100) -> list[Enrollment]:
        return [self.create_enrollment(choice(student_ids), choice(course_ids)) for _ in range(count)]
